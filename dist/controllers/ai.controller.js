...
async function getUserApiAuth(userId) {
    ...
    if (!user?.openaiKey)
        return null;
    return (0, apikey_service_1.getPrimaryProviderApiKey)(user.openaiKey);
}
...
async function requireConnectedApi(req, res) {
    const userApi = await getUserApiAuth(req.userId);
    if (!userApi) {
        res.status(400).json({
            error: "No API key connected. Add your API key in Settings to use modules.",
