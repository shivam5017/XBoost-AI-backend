...
export async function preLaunchOptimizer(
  draft: string,
  niche: string,
...
Be concise and tactical.`,
    ...
    analysis: {
      ...fallback,
      ...r.data,
      viralPotential: clampScore(Number((r.data as any)?.viralPotential ?? fallback.viralPotential)),
    },
    tokens: r.tokens,
  }));
}
    ...
    `Return JSON:
{
  ...
  "recommendations": string[]
    ...
    analysis: {
      ...fallback,
      ...r.data,
      trendSignals: ((r.data as any)?.trendSignals || fallback.trendSignals).map((t: any) => ({
...
${tweets.map((t, i) => `${i + 1}. ${t}`).join("\n")}`,
    ...
    analysis: {
      ...fallback,
      ...r.data,
      positioningScore: clampScore(Number((r.data as any)?.positioningScore ?? fallback.positioningScore)),
    },
    tokens: r.tokens,
  }));
}
...
Output a practical high-retention thread.`,
    `Topic: ${topic}
Objective: ${objective}`,
    fallback,
    userApiKey,
...
${draft}`,
    ...
    analysis: {
      ...fallback,
      ...r.data,
      score: clampScore(Number((r.data as any)?.score ?? fallback.score)),
    },
    tokens: r.tokens,
  }));
}
...
export async function contentPerformancePrediction(
  draft: string,
  niche: string,
  historicalBestHours: number[],
  userApiKey?: string | null,
